...
def get_last_periods():
    ...
    :return: int
    """
    r = requests.get("{}{}".format(UNION_LOTTO_URL, "history.shtml"))
    r.encoding = "gb2312"
...
def spider(start, end, mode):
    ...
    :return:
    """
    url = "{}{}".format(UNION_LOTTO_URL, UNION_LOTTO_PATH.format(start, end))
    ...
    if mode == "train":
        if start > 1:
            with open(UNION_LOTTO_FILE) as file:
                for row in csv.DictReader(file):
                    data.append(dict(row))
        df = pd.DataFrame(data)
        df.to_csv(UNION_LOTTO_FILE, mode='w', encoding="utf-8", index=True)
    elif mode == "predict":
        return pd.DataFrame(data)


if __name__ == "__main__":
    last = int(get_last_periods())
    print("最新一期期号：{}".format(last))
    if last <= 0:
        print('数据请求错误')
        exit(1)
    print("正在获取数据。。。")
    num = 0
    if os.path.exists(UNION_LOTTO_FILE):
        with open(UNION_LOTTO_FILE, 'r') as f:
            reader = csv.DictReader(f)
            num = int(next(reader)['期数'])
    print('num:', num)
    if num <= 0:
        spider(1, last, "train")
    elif 0 < num < last:
        spider(num + 1, last, "train")
    else:
        print('数据已经是最新')
